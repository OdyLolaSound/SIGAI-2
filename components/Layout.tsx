...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, onBack, goHome, hideNav, hideHeader, user }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen w-full md:max-w-none mx-auto bg-gray-50 md:shadow-none shadow-2xl relative overflow-hidden text-gray-900">
      {/* Header Premium - Tactical Designer Edition */}
      {!hideHeader && (
        <header className={`sticky top-0 z-40 transition-all duration-500 ${isMaster ? 'bg-tactical-orange shadow-[0_4px_20px_rgba(242,125,38,0.3)]' : 'bg-white border-b border-gray-100'}`}>
